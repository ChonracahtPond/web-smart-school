...
if (isset($_GET['select_all'])) {
    $stmt = $conn->prepare("SELECT * FROM register WHERE status_register IN (0, 1, 3)");
} else if ($start_date && $end_date) {
    $stmt = $conn->prepare("SELECT * FROM register WHERE status_register IN (0, 1, 3) AND registration_date BETWEEN ? AND ?");
    $stmt->bind_param('ss', $start_date, $end_date);
} else if ($year) {
    $stmt = $conn->prepare("SELECT * FROM register WHERE status_register IN (0, 1, 3) AND YEAR(registration_date) = ?");
...
<body>
    <div class="container mx-auto my-6 px-4">
        ...
        <form method="GET" action="your_script.php" class="mb-6 space-y-4">
            <div class="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4">
                <div class="flex-1">
                    <label for="start_date" class="block text-sm font-medium text-gray-700">วันที่เริ่มต้น</label>
                    <input type="date" name="start_date" id="start_date" class="mt-1 p-2 border border-gray-300 rounded-md shadow-sm w-full">
                </div>
                <div class="flex-1">
                    <label for="end_date" class="block text-sm font-medium text-gray-700">วันที่สิ้นสุด</label>
                    <input type="date" name="end_date" id="end_date" class="mt-1 p-2 border border-gray-300 rounded-md shadow-sm w-full">
                </div>
                <div class="flex-1">
            ...
            <div class="flex space-x-4 mt-4">
                ...
                <button type="submit" name="select_all" class="p-2 bg-green-500 text-white rounded-md shadow hover:bg-green-600 transition">เลือกทั้งหมด</button>
