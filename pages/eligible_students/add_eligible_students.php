...
function fetchStudents($conn, $grade_level = null)
{
    ...
    $eligible_students = [];
    $sql_check_eligible = "SELECT student_id FROM eligible_students WHERE eligible_type = 'class'";
...
{
    $sql_check = "SELECT COUNT(*) as count FROM eligible_students WHERE student_id = '" . $conn->real_escape_string($student_id) . "' AND eligible_type = 'class'";
...
<div class="p-8 mt-6 lg:mt-0 rounded-lg shadow bg-white">
    <h1 class="text-3xl font-semibold text-indigo-500 dark:text-white mb-5">เพิ่มนักศึกษาที่มีสิทธิสอบ </h1>
