function loadQuestionDetails(questionId) {
  ...
  fetch(
    `../pages/exercise/sql/get_question_details.php?question_id=${questionId}`
  )
    .then((response) => {
    ...
    .catch((error) => {
      ...
      const detailsDiv = document.getElementById("question-details");
      detailsDiv.innerHTML = `<div class="p-5 bg-red-200 text-red-800 rounded">เกิดข้อผิดพลาด: ${error.message}</div>`;
    });
}
