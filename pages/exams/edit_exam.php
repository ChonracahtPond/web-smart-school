<?php
// เชื่อมต่อฐานข้อมูล (กรุณาแก้ไขให้ตรงกับการเชื่อมต่อของคุณ)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (strpos($criterion1, '%') !== false) { // แก้ไขเป็น $criterion1
        // ลบเครื่องหมาย '%' ออก
        $cleanedCriterion = str_replace('%', '', $criterion1);
        $total1 = $total_marks1 * ($cleanedCriterion / 100);
        // $total = $total_marks1 * ($cleanedCriterion / 100);
        if ($score >=  $total1) {
            $status1 = "ผ่าน";
        }
    } else {
        $cleanedCriterion = $criterion1; // ใช้ $criterion1
        $total = 0; // หรือค่าที่คุณต้องการกำหนดเมื่อไม่มีการคำนวณ
    }

    // แสดงผลข้อมูลที่รับมา
    echo "<div class='mb-4'>";
    echo "<strong>คะแนนเต็ม:</strong> " . htmlspecialchars($total_marks1) . "<br>";
    echo "<strong>คะแนนจากการสอบ:</strong> " . htmlspecialchars($score1) . "<br>";
    echo "<strong>เกณฑ์การผ่าน:</strong> " . htmlspecialchars($criterion1) . "<br>";
    // echo "<strong>คะแนนที่ต้องการผ่าน:</strong> " . htmlspecialchars($totalPassingCriteria) . "<br>";
    echo "<strong>คะแนนที่ต้องการผ่าน:</strong> " . htmlspecialchars($total1) . "<br>";
    echo "<strong>ผ่านไหม:</strong> " . htmlspecialchars($status1) . "<br>";
    echo "</div>";
    ...
    $updateSQL = "UPDATE exams SET exam_type = ?, exam_date = ?, duration = ?, total_marks = ?, score = ? WHERE exam_id = ?";
    $stmt = $conn->prepare($updateSQL);
    $stmt->bind_param("ssissi", $exam_type, $exam_date, $duration, $total_marks1, $score1, $exam_id);
