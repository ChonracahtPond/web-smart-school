...
$sql_borrowings = "SELECT b.borrowing_id, i.item_name, b.quantity, u.first_name AS user_id
        FROM borrowings b
            ...
            <ul id="itemsList" class="list-disc pl-5 space-y-2 overflow-y-auto max-h-72 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
                <?php while ($row = $items_result->fetch_assoc()) { ?>
                    <li class="flex items-center justify-between p-2 hover:bg-gray-100 rounded-lg transition-all">
                        <div class="flex-1">
                            <button type="button" onclick="selectItem('<?php echo $row['item_id']; ?>', '<?php echo $row['item_name']; ?>', <?php echo $row['quantity']; ?>)" class="text-blue-600 hover:underline">
                        ...
                        <button type="button" onclick="addItemToBorrowList('<?php echo $row['item_id']; ?>', '<?php echo $row['item_name']; ?>', <?php echo $row['quantity']; ?>)" class="bg-green-600 text-white px-3 py-1 rounded-lg hover:bg-green-700 transition-all">
                            เบิก
                        </button>
                    </li>
                <?php } ?>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        window.selectItem = function(itemId, itemName, itemQuantity) {
            // Add the selected item to the list for borrowing
            addItemToBorrowList(itemId, itemName, itemQuantity);
        }

        window.addItemToBorrowList = function(itemId, itemName, itemQuantity) {
            ...
            if (!existingItem) {
                ...
                <div id="item-${itemId}" class="flex items-center justify-between border-b border-gray-300 pb-2 mb-2">
                    ...
                    <input type="number" name="quantity_${itemId}" class="form-input w-24 border-gray-300 rounded-lg" min="1" max="${itemQuantity}" placeholder="จำนวน" required>
        ...
        function updateHiddenInput() {
            ...
            items.forEach(item => {
                ...
                if (quantity > 0) { // Only add to data if quantity is greater than 0
                    itemsData.push({
                        id: itemId,
                        quantity: quantity
                    });
